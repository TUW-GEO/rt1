...
def start_log_to_file(path, name='rt1_filehandler', level=logging.INFO):
    ...
    try:
        ...
        hc = [val for val in log.handlers
              if val.name == 'rt1_consolehandler'][0]
        ...
        log.debug(f'log-file for handler {name} added at location' +
                  f' "{fh.baseFilename}"!')
...
def stop_log_to_file(name='rt1_filehandler'):
    ...
    if name in handlers:
        for i, h in enumerate(handlers[name]):
            h.close()
            log.removeHandler(h)
            log.debug(f'{i + 1} existing logfile-handler "{name}" at' +
...
class WrappedFixedIndentingLog(logging.Formatter):
    ...
    def format(self, record):
        return indent(super().format(record),
                      ' '*self._indent).strip()
