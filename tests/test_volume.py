...
class TestVolume(unittest.TestCase):
    ...
    def test_rayleigh(self):
        ...
        self.assertEqual(p, (3.0 / (16.0 * np.pi)))
    ...
    def test_HenyeyGreenstein(self):
        ...
        self.assertAlmostEqual(
            p, (1.0 - 0.7 * 0.7) / (4.0 * np.pi * (1.0 + 0.7 * 0.7) ** 1.5), 10
        )
    ...
    def test_HGRayleigh(self):
        ...
        self.assertAlmostEqual(
            p, 4.0 * np.pi * (4 / (2 + 0.7**2)) * 1 / 2 * phg * pr, 10
        )

    def test_HenyeyGreenstein_coefficients(self):
        V = HenyeyGreenstein(omega=0.2, tau=1.7, t=0.7, ncoefs=20)
        init_dict = V.init_dict
        self.assertEqual(V._get_legcoef(0), 1.0 / (4.0 * np.pi))
        self.assertEqual(V._get_legcoef(1), 3.0 * 0.7 / (4.0 * np.pi))
    ...
    def test_V_normalization(self):
        ...
        V4 = LinCombV(
            [[0.5, Rayleigh()], [0.5, HenyeyGreenstein(t=0.7, ncoefs=20)]],
            omega=0.2,
            tau=1.7,
        )
        ...
        for V in [V1, V2, V3, V4]:
            ...
            for theta_0, phi_0 in product(incnum, pincnum):
                ...
                def integfunkt(theta_s, phi_s):
                    return np.sin(theta_s) * V.p(theta_0, theta_s, phi_0, phi_s)

                # evaluate the integral using Simpson's Rule twice
                z = integfunkt(x[:, None], y)
                sol += [simps(simps(z, y), x)]

                self.assertTrue(np.allclose(sol, 1.0))

    def test_LinCombV(self):
        Vr = Rayleigh(omega=0.2, tau=1.7)
        Vhg = HenyeyGreenstein(omega=0.2, tau=1.7, t=0.7, ncoefs=20)

        V = LinCombV([[0.5, Vr], [0.5, Vhg]], omega=0.2, tau=1.7)
        ...
        for t_0, t_ex, p_0, p_ex in product(incnum, pincnum, incnum, pincnum):
            ...
            self.assertAlmostEqual(p, 0.5 * (phg + pr), 10)
