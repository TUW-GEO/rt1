    ...
    return (
        ...
        * (1.0 - t**2)
        / (
            ...
            * t
            * (
                np.cos(thetai) * np.cos(thetas)
                + np.sin(thetai) * np.sin(thetas) * np.cos(phii - phis)
            )
        )
        ** (3.0 / 2.0)
    )
...
CosP = 5.0  # set the power of the cosine-lobe
HGt = 0.7  # set the asymmetry parameter of the Henyey Greenstein Function
phii = 0.0  # set the incident direction to phi_i = 0.
...
inc = np.arange(5.0, 90.0, 10.0)

# define grid for integration
x = np.linspace(0.0, np.pi / 2.0, 3000)
y = np.linspace(0.0, 2 * np.pi, 3000)
...
for thetai in np.deg2rad(inc):
    ...
    def integfunkt(thetas, phis):
        return np.array(
            ...
            * np.cos(thetas)
            / (mu0 - np.cos(thetas))
            * (np.exp(-tau / mu0) - np.exp(-tau / np.cos(thetas)))
            * RAYLEIGH(thetai, thetas, phii, phis)
            * COSLOBE(thetai, np.pi - thetas, np.pi, phis, CosP)
        )
    ...
    def integfunkt(thetas, phis):
        return np.array(
            ...
            * np.cos(thetas)
            / (mu0 - np.cos(thetas))
            * (np.exp(-tau / mu0) - np.exp(-tau / np.cos(thetas)))
            * HG(thetai, thetas, phii, phis, HGt)
            * COSLOBE(thetai, np.pi - thetas, np.pi, phis, CosP)
        )
...
np.savetxt(
    "../tests/example1_int.csv", [[i, j] for i, j in zip(inc, solCosRay)], delimiter=","
)
np.savetxt(
    "../tests/example2_int.csv", [[i, j] for i, j in zip(inc, solCosHG)], delimiter=","
)
